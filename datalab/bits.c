...
int howManyBits(int x) {
  ...
  int sign=x>>31;
  x = (sign&~x)|(~sign&x);// 若 x 為則不變，否則則按位取反（這樣比較好找最高位為 1 的，也就是原來是最高位為 0 的，這樣也將符號位置去掉了）

  // 不斷縮小範圍
  b16 = !!(x>>16)<<4; // 高十六位是否有 1
  x = x>>b16;         // 如果有（至少需要16位），則將原數右移16位
  b8 = !!(x>>8)<<3;   // 剩餘位高 8 位是否有 1
  ...
  x = x>>b1;
  b0 = x;
  return b16+b8+b4+b2+b1+b0+1;// +1 表示加上符號位
}
//float
/* 
